...
For environment setup, running tests, and opening PRs, start with [CONTRIBUTING.md](../CONTRIBUTING.md). This guide covers project structure and how to extend the integration.
...
1. **Data source** — Add or reuse an API method in [`api.py`](../custom_components/jullix/api.py) and a path constant in [`const.py`](../custom_components/jullix/const.py) if needed. See [api.md](api.md) for the existing path map.
...
3. **Entity** — Implement a sensor class (often subclassing **`JullixSensor`** in [`sensors/base.py`](../custom_components/jullix/sensors/base.py)) or add a factory in the appropriate module under **`sensors/`** (see [`sensors/cost.py`](../custom_components/jullix/sensors/cost.py), [`sensors/energy.py`](../custom_components/jullix/sensors/energy.py), or [`sensors/battery.py`](../custom_components/jullix/sensors/battery.py) for battery cumulative energy).
4. **Registration** — Wire the factory into [`sensors/setup.py`](../custom_components/jullix/sensors/setup.py) (`async_setup_entry`), gated by options if the sensor is optional. If the sensor instead depends on whether the API actually returned data (not a user-facing option), call it unconditionally and gate entity creation on data presence inside the factory itself — see [`sensors/algorithm_extended.py`](../custom_components/jullix/sensors/algorithm_extended.py) and [`sensors/charger_extended.py`](../custom_components/jullix/sensors/charger_extended.py).
...
5. Document the path in [api.md](api.md).

## Testing

See [tests/README.md](../tests/README.md) for pytest commands, CI behavior, and API fixtures.
